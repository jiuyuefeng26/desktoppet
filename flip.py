...
class Player:
    def __init__(self, root: tk.Tk) -> None:
        ...
        with Image.open("plantcell20%.png") as original_image:          
            # 处理白色背景为透明（在图像层面）
            self.original_image = self.make_background_transparent(original_image)
    ...
    def tick(self) -> None:
        ...
        if self.walking and self.drag_offset is None:
            ...
            if self.x <= 0:
                self.x = 0.0
                self.direction_x = 1  # 改变方向为向右
                self.flip_image_horizontal()  # 水平翻转
            elif self.x + self.width >= self.screen_w:
                self.x = float(self.screen_w - self.width)
                self.direction_x = -1  # 改变方向为向左
                self.flip_image_horizontal()  # 水平翻转
            
            # 边界碰撞检测 - Y轴（垂直翻转 + 改变方向）
            if self.y <= 0:
                self.y = 0.0
                self.direction_y = 1  # 改变方向为向下
                self.flip_image_vertical()  # 垂直翻转
            elif self.y + self.height >= self.screen_h:
                self.y = float(self.screen_h - self.height)
                self.direction_y = -1  # 改变方向为向上
                self.flip_image_vertical()  # 垂直翻转
